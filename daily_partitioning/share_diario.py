...
def share_diario(df_parametro, #df que será a base
    ...
    df_parametro['day'] = df_parametro[coluna_datas].dt.day_name() #Inclusão da coluna dia da semana. Com o nome do dia em inglês
    ...
    df_bases_totais['day'] = days[0] #como a base não tem mais kpis, transformamos todos em sunday. Para em seguida fazer o loop de adicionar todos os dias.
    ...
    df_days = df_parametro.groupby([coluna_semanas] + aberturas + ['day'], as_index=False)[etapas_vol].sum() #groupby para transformar a base em um df de dias da semana
    df_share = pd.merge(df_days, df_weeks, how='left', on = [coluna_semanas] + aberturas, suffixes= ('_day', '_week')) #merge entre os dois dfs acima para que tenhamos em um mesmo df o valor total da semana e o do dia.
    df_share = pd.merge(df_bases_totais, df_share, how = 'left', on = aberturas + ['day']) #merge da base com todas aberturas possíveis com os kpis
    ...
    df_share = df_share.groupby(aberturas+['day'], as_index=False)[etapas_share].mean() #média dos percentuais de todos os week_starts
    ...
    for etapa in etapas_share: #Tudo que é menor que o mínimo, vira o mínimo permitido. Tratamos fins de semanas com pesos diferentes
      df_share.loc[( (df_share['day'] == 'Sunday') & (df_share[etapa] < min_share) ), [etapa]] = 1./78
      df_share.loc[( (df_share['day'] == 'Saturday') & (df_share[etapa] < min_share) ), [etapa]] = 1./10
      df_share.loc[( (df_share['day'] != 'Sunday') & (df_share['day'] != 'Saturday') & (df_share[etapa] < min_share) ), [etapa]] = 1.0/7
